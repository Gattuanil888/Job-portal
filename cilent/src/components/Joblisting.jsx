...
function Joblisting() {
  ...
  useEffect(() => {
    ...
    const matchesCategory = job =>
      ...
      selectedCategories.map(cat => cat.toLowerCase()).includes(job.category.toLowerCase());

    const matchesLocation = job =>
      selectedLocations.length === 0 ||
      selectedLocations.map(loc => loc.toLowerCase()).includes(job.location.toLowerCase());
  ...
  const categories = ["Programming", "Marketing", "Designing", "Accounting", "Analytics", "Development"];
const locations = ["Hyderabad", "Bangalore", "Mumbai", "Chennai", "Remote"];
        ...
        <div className={`${showFilter ? '' : 'hidden'} lg:block`}>
          ...
          {categories.map((cat, idx) => (
            <div key={idx} className="flex items-center mb-2">
              ...
              />
              <label className="text-sm text-gray-700 ml-2">{cat}</label>
            </div>
          ))}
          ...
          {locations.map((loc, idx) => (
            <div key={idx} className="flex items-center mb-2">
              ...
              />
              <label className="text-sm text-gray-700 ml-2">{loc}</label>
            </div>
          ))}
        </div>
      </div>
